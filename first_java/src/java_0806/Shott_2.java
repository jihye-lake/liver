...
class GamePanel extends JPanel{
    ...
    GamePanel(){
        ...
        ImageIcon img = new ImageIcon("src/images/snack_bite.png");
    ...
    public void startGame(){
        base.setLocation(this.getWidth()/2-20, this.getHeight()-60);
        bullet.setLocation(this.getWidth()/2-5, this.getHeight()-70);
        ...
        public void run(){
            while(true){
                if(hit()){//타겟이 맞았다면
                    ...
                    int x=bullet.getX();
                    int y=bullet.getY()-7;//5픽셀씩 위로 이동한다.=총알 속도가 5픽셀
                    //총알이 프레임 밖으로 나갔을 때
                    if(y<0){
                        //총알 원래 위치로 이동
                        bullet.setLocation(bullet.getParent().getWidth()/2-5, bullet.getParent().getHeight()-50);
